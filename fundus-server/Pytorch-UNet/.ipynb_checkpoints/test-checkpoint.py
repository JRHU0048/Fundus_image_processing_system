...
if __name__ == '__main__':
    ...
    args.test_dir_img = '/root/autodl-tmp/3dataset/image256/'
    args.test_dir_mask = '/root/autodl-tmp/3dataset/mask3_256/'

    # args.test_dir_img = '/root/autodl-tmp/Harvard_seg/Testing/image/'
    # args.test_dir_mask = '/root/autodl-tmp/Harvard_seg/Testing/mask3CLASS/'
    
    args.pretrained_model_path = './checkpoints/checkpoint_epoch_2.pth'
    # 0 是 3dataset模型； 1是 harvard 模型  2是cut训练的模型
    
    # 加载预训练模型
    model = UNet(n_channels=3, n_classes=args.classes)
    state_dict = torch.load(args.pretrained_model_path, map_location=device)
    # 若训练保存模型包含'mask_values'，此处需谨慎处理，可能删除或转换以适配测试模型加载
