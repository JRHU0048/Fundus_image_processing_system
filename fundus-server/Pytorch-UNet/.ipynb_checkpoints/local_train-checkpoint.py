...
#=====================================dataset===================================
# harvard dataset
# dir_img = Path('/root/autodl-tmp/Harvard_seg/Training/image/')
# dir_mask = Path('/root/autodl-tmp/Harvard_seg/Training/mask3CLASS/')

# 3dataset
# dir_img = Path('/root/autodl-tmp/3dataset/image/')
# dir_mask = Path('/root/autodl-tmp/3dataset/mask3/')

# CUT: harvard->3dataset
dir_img = Path('/root/autodl-tmp/cut_data_seg/grumpycat_CUT_unet/train_latest/images/fake_B/')
dir_mask = Path('/root/autodl-tmp/Harvard_seg/Training/mask3CLASS256/')
#=====================================dataset===================================
...
def train_model(
    ...
    # 4. 设置优化器、损失函数、学习率调度器等（重新设置，每个“站点”独立）
...
def get_args():
    ...
    parser.add_argument('--classes', '-c', type=int, default=3, help='Number of classes')   # 青光眼分割是三类！！！
...
if __name__ == '__main__':
    ...
    for p in range(global_epoch):
        ...
        if True:
            ...
            torch.save(state_dict, str(dir_checkpoint / f'checkpoint_epoch_2.pth'))
            logging.info(f'Checkpoint {p + 1} saved!')